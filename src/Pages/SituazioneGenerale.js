...
class SituazioneGenerale extends React.Component {
    ...
    componentWillMount = () => {
        ...
        fetchAsync(urlItalia)
            .then(data => {
                ...
                console.log(this.state.dataStato);
            })
         ...
         fetchAsync(urlRegioni)
            .then(data => {
                ...
                console.log(this.state.dataRegioni);
            })
         ...
         fetchAsync(urlProvincie)
            .then(data => {
                ...
                console.log(this.state.dataProvincie);
            })
    ...
    componentDidMount = () => {
        ...
        if (node instanceof HTMLElement) {
            child = node.querySelectorAll('path');
        }

        setTimeout(()=>{
            child.forEach(pathRegioneCorrente => {
            
                console.log(pathRegioneCorrente);
                const nome = pathRegioneCorrente.getAttribute('title');

                let datiTrentino = 0;

                this.state.dataRegioni.forEach((datoRegione) => {
                    if (datoRegione.denominazione_regione===nome && datoRegione.denominazione_regione!=='P.A. Bolzano' && datoRegione.denominazione_regione!=='P.A. Trento'){
                        if (datoRegione.totale_positivi<=100){
                            pathRegioneCorrente.setAttribute('fill','cornsilk');
                        }
                        else if(datoRegione.totale_positivi>100 && datoRegione.totale_positivi<=500 ){
                            pathRegioneCorrente.setAttribute('fill','#FFD29D');               
                        }
                        else if(datoRegione.totale_positivi>500 && datoRegione.totale_positivi<=3000 ){
                            pathRegioneCorrente.setAttribute('fill','#FFB563');               
                        }
                        else if(datoRegione.totale_positivi>3000 && datoRegione.totale_positivi<=10000 ){
                            pathRegioneCorrente.setAttribute('fill','#F85E00');               
                        }
                        else if(datoRegione.totale_positivi>10000){
                            pathRegioneCorrente.setAttribute('fill','#B3001B'); 
                        }
                    }

                    else{
                        datiTrentino+=datoRegione.totale_positivi
                    }
                ...
                if (nome==='Trentino Alto Adige'){
                    if (datiTrentino<=100){
                        pathRegioneCorrente.setAttribute('fill','lightgreen');
                    }
                    else if(datiTrentino>100 && datiTrentino<=500 ){
                        pathRegioneCorrente.setAttribute('fill','green');               
                    }
                    else if(datiTrentino>500 && datiTrentino<=3000 ){
                        pathRegioneCorrente.setAttribute('fill','pink');               
                    }
                    else if(datiTrentino>3000 && datiTrentino<=10000 ){
                        pathRegioneCorrente.setAttribute('fill','red');               
                    }
                    else if(datiTrentino>10000){
                        pathRegioneCorrente.setAttribute('fill','darkred'); 
                    }
                }
    ...
    onClickHandlerMap = async (event) => {     
        await this.setState({
            ...this.state,
            regioneAttiva: event.getAttribute('title'),
        });
        console.log('attiva: ', this.state.regioneAttiva);
        this.state.dataRegioni.forEach((datoRegione) => {
            if(datoRegione.denominazione_regione===this.state.regioneAttiva){
                console.log('yes');
                this.setState({
                    ...this.state,
                    datiRegioneAttiva:datoRegione,
                })
            }
        })

        console.log(this.state.datiRegioneAttiva);      
        ...
        return (
          <React.Fragment>
            {this.state.isMounted && (
              <div className="contenitore_situazione-generale">
                <h2>
                  Ultimo aggiornamento:{" "}
                  {this.state.dataStato.data
                    ? invertiData( this.state.dataStato.data.substring(0, 10) )
                    : null}{" "}
                  alle ore:{" "}
                  {this.state.dataStato.data
                    ? this.state.dataStato.data.substring(11,16)
                    : null}{" "}
                </h2>
                <div className="situazione_italia">
                  <h1>Italia</h1>
                  <div className="situazione_italia_dati">
                    <div className="italia_totale-casi italia_info">
                      <h2>{this.state.dataStato.totale_casi}</h2>
                      <h3>Totale casi</h3>
                    </div>
                    <div className="italia_totale_positivi italia_info">
                      <h2 style={this.state.dataStato.variazione_totale_positivi < 0 ? {color:'green'} : {color:'red'} }>({this.state.dataStato.variazione_totale_positivi})</h2>
                      <h2>{this.state.dataStato.totale_positivi}</h2>
                      <h3>Totale positivi</h3>
                    </div>
                    <div className="italia_nuovi-positivi italia_info">
                      <h2>{this.state.dataStato.nuovi_positivi}</h2>
                      <h3>Nuovi positivi</h3>
                    </div>
                    <div className="italia_deceduti italia_info">
                      <h2>{this.state.dataStato.deceduti}</h2>
                      <h3>Decessi</h3>
                    </div>
                    <div className="italia_terapia-intensiva italia_info">
                      <h2>{this.state.dataStato.terapia_intensiva}</h2>
                      <h3>Terapia intensiva</h3>
                    </div>
                    <div className="italia_isolamento italia_info">
                      <h2>{this.state.dataStato.isolamento_domiciliare}</h2>
                      <h3>Isolamento domiciliare</h3>
                    </div>
                  </div>
                </div>
                <div className='contenitore_totale_regioni'>
                <div className="situazione_regioni">
                  <h1>REGIONI</h1>
                  <div className="contenitore_tabella_regioni">
                    <table>
                      <thead>
                        <tr>
                          <th>Stato</th>
                          <th>Casi totali</th>
                          <th>Positivi</th>
                          <th>Variazione positivi</th>
                          <th>Tamponi effettuati</th>
                          <th>Terapia intensiva</th>
                          <th>Isolamento domiciliare</th>
                        </tr>
                      </thead>
                      {this.state.dataRegioni
                        ? this.state.dataRegioni.map((regione) => {
                            return (
                              <tbody key={regione.denominazione_regione}>
                                <tr>
                                  <td
                                    style={{
                                      fontSize: "1rem",            
                                  ...
                                  <td>{regione.totale_casi}</td>
                                  <td className='aggiustare'>{regione.totale_positivi} {regione.nuovi_positivi!==0?`(+${regione.nuovi_positivi})`:''}</td>        
                                  <td>{regione.variazione_totale_positivi}</td>
                                  <td>{regione.tamponi}</td>
                                  <td>{regione.terapia_intensiva}</td>
                                  <td>{regione.isolamento_domiciliare}</td>
                                </tr>
                              </tbody>
                            );
                          })
                ...
                {/*<div className='situazione_provincie'>
                            <h1>PROVINCIE</h1>
                        </div>*/}
                ...
                this.state.regioneAttiva && <div className='informazioni_regione_cliccata'>
                      ...
                      <div className='info_contenitore_quadrati'>
                        <div className='info_quadrato_regione'>
                            <h2>Totale casi</h2>
                            <h2>{this.state.datiRegioneAttiva.totale_casi}</h2>
                        </div>
                        <div className='info_quadrato_regione'>
                            <h2>Totale positivi</h2>
                            <h2>{this.state.datiRegioneAttiva.totale_positivi}</h2>
                        </div>
                        <div className='info_quadrato_regione'>
                            <h2>Nuovi positivi</h2>
                            <h2>{this.state.datiRegioneAttiva.nuovi_positivi}</h2>
                        </div>
                        <div className='info_quadrato_regione'>
                            <h2>Decessi</h2>
                  ...
                  <svg
                    ...
                    mapsvggeoviewbox="6.624486 47.092916 18.521301 35.490303"
                    width="610.30981"
                    height="792.58575"
                  >
                    <path
                      onClick={e=>this.onClickHandlerMap(e.currentTarget)}
...
export default SituazioneGenerale;